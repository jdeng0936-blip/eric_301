...
/* SDCC 4.5.0 workaround: ADCx->TRIGR[n] triggers ICE */
#ifdef __SDCC
#define ADC_TRIGR(adc, n) (*((volatile uint8_t *)((uint16_t)(adc) + 14u + (n))))
#else
#define ADC_TRIGR(adc, n) ((adc)->TRIGR[(n)])
#endif
...
void ADC_DeInit(ADC_TypeDef* ADCx)
{
  ...
  ADC_TRIGR(ADCx, 0) =  ADC_TRIGR1_RESET_VALUE;
  ADC_TRIGR(ADCx, 1) =  ADC_TRIGR2_RESET_VALUE;
  ADC_TRIGR(ADCx, 2) =  ADC_TRIGR3_RESET_VALUE;
  ADC_TRIGR(ADCx, 3) =  ADC_TRIGR4_RESET_VALUE;
}
...
void ADC_TempSensorCmd(FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ...
    ADC_TRIGR(ADC1, 0) |= (uint8_t)(ADC_TRIGR1_TSON);
  }
  else
  {
...
void ADC_VrefintCmd(FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ...
    ADC_TRIGR(ADC1, 0) |= (uint8_t)(ADC_TRIGR1_VREFINTON);
  }
  else
  {
                              ...
                              FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ...
    ADC_TRIGR(ADCx, regindex) &= (uint8_t)(~(uint8_t)ADC_Channels);
  }
  else
  {
    /* Disable the Schmitt Trigger for the selected ADC channel(s).*/
    ADC_TRIGR(ADCx, regindex) |= (uint8_t)(ADC_Channels);
  }
}

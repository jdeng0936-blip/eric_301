...
void IIC_Init(void)//当A2为底电平时，4页的设备地址(页地址，查找页)分别为a0H，a2H，a4H，a6H
{
    ...
    Load_Stat((uint8_t *)&stt);//读取统计数据
    if ((stt[A].tscount == 0xffff) && (stt[B].tscount == 0xffff) && (stt[C].tscount == 0xffff)) //如果是首次读取，先初始化
        memset((uint8_t *)&stt, 0, 70);

    Delay(IIC_TIMEOUT);
    adjustdt.dt[11] = I2C_ReadByte(0xa0, 80); //语言选择
    language = adjustdt.dt[11];

    Delay(IIC_TIMEOUT);
    data_index.w = I2C_ReadByte(0xa0, 250); //读取保存了的数据索引
    if (data_index.w == 0xff)
        data_index.w = 0;
    Delay(IIC_TIMEOUT);
    data_index.used = I2C_ReadByte(0xa0, 251);
    if (data_index.used == 0xff)
        data_index.used = 0;

    id[0] = I2C_ReadByte(0xa0, 252);
    Delay(IIC_TIMEOUT);
    id[1] = I2C_ReadByte(0xa0, 253);
    Delay(IIC_TIMEOUT);
    id[2] = I2C_ReadByte(0xa0, 254);
    Delay(IIC_TIMEOUT);
    id[3] = I2C_ReadByte(0xa0, 255);

    selfid = id[0] * 1000000 + id[1] * 10000 + id[2] * 100 + id[3];
    if (selfid > 99999999)
        selfid = 1;
    r485id = (selfid & 0xff);
}
...
void init_rtc(void)
{
    ...
    if (tag != 0x55)
    {
        ...
        RTC_DateStr.RTC_Date = 16;
        RTC_DateStr.RTC_Month = RTC_Month_March;
        RTC_DateStr.RTC_Year = 24;
